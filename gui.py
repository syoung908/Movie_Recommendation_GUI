...
class MainScreen(Screen):
    ...
    def go_to_search(self):
        ...
        self.manager.get_screen('search').load_searches()
...
class SearchScreen(Screen):
    def __init__(self, **kwargs):
        ...
        self.search_term = ''
        self.search_queue = Deque['Movie_Data']()
        self.search_view = Label(pos_hint={'center_x': 0.5, 'center_y': 0.5},
            font_size=30, text='No Results')
        button_panel = BoxLayout(orientation='horizontal', padding=(10,10,10,10),
            height=120, size_hint=(1, None), spacing=10)
        self.add_widget(self.search_view)
        self.back_button = CustomButton('gui_assets/go_back.png', 'Go Back')
        self.back_button.bind(on_press=self.go_back)
        self.back_button.pos_hint={'x': 0, 'y': 0}
        button_panel.add_widget(self.back_button)

        self.search_label = Label(pos_hint={'center_x': 0.5, 'center_y': 0.9},
            font_size=30, text='Search Results')

        self.add_widget(self.search_label)
        self.add_widget(button_panel)

    def load_searches(self):
        self.remove_widget(self.search_view)
        self.loading_wheel = Loading()
        self.search_label.text = 'Results Containing: \"' + self.search_term + '\"' 
        self.loading_message = LoadingMessage(text='Searching Movie Database')
        self.add_widget(self.loading_wheel)
        self.add_widget(self.loading_message)
        Thread(target=self.generate_search).start()

    def generate_search(self):
        results = movies[movies['title'].str.contains(self.search_term, 
            case=False, regex=False)]
        ids = results['movieId']

        for res in ids:
            self.search_queue.append(MovieData(res))
        ...
        if len(self.search_queue) == 1:
            self.manager.transition.direction = 'right'
            self.manager.current = 'main'
            self.manager.get_screen('main').current_movie=self.search_queue.popleft()
            self.manager.get_screen('main').refresh()
    ...
    def update_search_window(self):
        self.search_view = InfinityScrollView(self.search_queue)
        self.search_view.bind(on_select_movie=self.view_movie_details)
        self.add_widget(self.search_view)
...
class SearchPopUp(Popup):
    def __init__(self, parent_panel):
        super(SearchPopUp, self).__init__(title='Search For Movies', separator_height=0,
            size_hint=(None, None), size=(500,420), title_size=20,
            background_color=(0, 0, 0, 0))

        self.layout= BoxLayout(orientation='vertical')
        button_panel = BoxLayout(orientation='horizontal', padding=(10,10,10,10),
            height=40, size_hint=(1, None), spacing=10)

        anchor = AnchorLayout(anchor_x='center', anchor_y='top', padding=(0,30,0,0))
        self.parent_panel = parent_panel
        self.auto_dismiss = True
        self.search_bar = AutoCompleteTextInput(size_hint = (None, None), 
            size = (450, 60), pos_hint={'center_x': .5, 'center_y': .5})
        self.search_bar.ids.txt_input.word_list = movies.title.values 
        self.enter_button = Button(text='Search', font_size=15, size=(60, 40),
            size_hint=(None, None))
        self.enter_button.bind(on_press=self.search)

        button_panel.add_widget(self.enter_button)

        anchor.add_widget(self.search_bar)
        self.layout.add_widget(anchor)
        self.layout.add_widget(button_panel)
        self.add_widget(self.layout)
    
    def search(self, obj):
        self.parent_panel.manager.get_screen('search').search_term = self.search_bar.get_text()
        self.parent_panel.go_to_search()
        self.dismiss()
